...
for (const site of SITES) {
  ...
  console.log(
    `  playwright: ${playwrightPages.length} pages in ${playwrightMs}ms  (mem: ${pm1}→${pm2} MB, Δ${pm2 - pm1} MB)`,
  );
}
...
const avgPMem = Math.round(
  results.reduce((s, r) => s + (r.playwright.memEnd - r.playwright.memStart), 0) / results.length,
);
console.log('─'.repeat(100));
console.log(
  `${'TOTALS'.padEnd(28)} ${String(totalCPages).padStart(7)} ${String(totalCMs).padStart(7)} ${(avgCMem + ' avg').padStart(8)} ${String(totalPPages).padStart(8)} ${String(totalPMs).padStart(8)} ${(avgPMem + ' avg').padStart(9)}`,
);
