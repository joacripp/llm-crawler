...
export function generateLlmsTxt(pages: PageData[], rootUrl: string): string {
  ...
  const rootPage = pages.find((p) => { const u = new URL(p.url); return u.pathname === '/' || u.pathname === ''; }) ?? pages[0];
  const siteTitle = extractSiteTitle(rootPage.title, new URL(rootUrl).hostname);
  const lines: string[] = [];
  lines.push(`# ${siteTitle}`);
  lines.push('');
  if (rootPage.description) { lines.push(`> ${rootPage.description}`); lines.push(''); }
  const nonRoot = pages.filter((p) => p !== rootPage);
  if (nonRoot.length === 0) { lines.push(`- [${rootPage.title}](${rootPage.url})`); return lines.join('\n'); }
  ...
  const sorted = [...sectionMap.entries()].sort(([a], [b]) => {
    const ai = PRIORITY.indexOf(a.toLowerCase()); const bi = PRIORITY.indexOf(b.toLowerCase());
    if (ai !== -1 && bi !== -1) return ai - bi;
    if (ai !== -1) return -1; if (bi !== -1) return 1;
    return a.localeCompare(b);
  });
  for (const [section, sectionPages] of sorted) {
    lines.push(`## ${section}`); lines.push('');
