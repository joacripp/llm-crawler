...
export async function handler() {
    ...
    if (!queueUrl)
        throw new Error('JOBS_QUEUE_URL env var is required');
    const completedQueueUrl = process.env.COMPLETED_QUEUE_URL;
    if (!completedQueueUrl)
        throw new Error('COMPLETED_QUEUE_URL env var is required');
    ...
    try {
        ...
        for (const job of staleJobs) {
            ...
            const message = { jobId: job.id, urls: pending, visited, maxDepth: job.maxDepth, maxPages: job.maxPages };
